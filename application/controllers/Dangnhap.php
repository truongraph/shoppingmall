...
class Dangnhap extends CI_Controller {
    ...
    //         }
    //     }else{
    //         $this->data['title']='Hàng Chính Hiệu - Đăng nhập tài khoản';
    //         $this->data['view']='dangnhap';
    //         $this->load->view('frontend/layout',$this->data);
    //     }     
    // }
    ...
    public function dangnhap()
	{
        ...
        if ($this->form_validation->run() ==TRUE)
        {
        	...
        	else
	        {
	        	...
	        	$this->load->view('frontend/components/dangnhap/dangnhap', $data);
	        }
        }
        else
        {
        	$this->load->view('frontend/components/dangnhap/dangnhap');
        }
	}
    ...
    public function dangky(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $config['smtp_user']    = 'truong.vd2000@gmail.com';
            $config['smtp_pass']    = 'uerhehipolmuwlnk';
            ...
            $this->email->from('truong.vd2000@gmail.com', 'Hàng Chính Hiệu');
            $this->email->to($email);
            $this->email->subject('Hàng Chính Hiệu - Quà thành viên mới');
            $this->email->message('<h3>ĐĂNG KÝ TÀI KHOẢN THÀNH CÔNG</h3>
                                    <p>Bạn đã trở thành thành viên mới của Hàng Chính Hiệu.!</p>
                                    <p>Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000đ : <a><strong>'.$tempcoupon.'</strong></a>
                                    <p>Mã này có giá trị tới ngày: '.$tempdatelimit.'</p>
                                    Hãy sử dụng tài khoản để mua hàng để tích lũy nhận thêm nhiều ưu đãi.!</p>
                ');
            $this->email->send();
            $this->data['success']='Đăng ký thành công. Bạn đã nhận được 1 mã giảm giá cho thành viên mới, vui lòng kiểm tra email';

        }  
        $this->data['title']='Hàng Chính Hiệu - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->subject('Hàng Chính Hiệu - Lấy lại mật khẩu');
            $this->email->message(
                '<strong>Xin chào bạn.</strong><br>
                <p>Chúng tôi có nhận yêu cầu khôi phục tài khoản từ hệ thống cửa hàng</p>
                <p><strong style="color:red">Lưu ý: Không chia sẻ link này cho bất cứ ai !</strong></p>
                <p>Dưới đây là link để thay đổi mật khẩu:</p>
                <a href="'.base_url().'dangnhap/reset_password_new/'.$list['id'].'">'.base_url().'dangnhap/reset_password_new/'.$list['id'].'</a>
                <p style="margin-top:20px">Chúng tôi xin cảm ơn bạn đã tin tưởng chúng tôi</p>
                '); 
                
            $this->email->send();
            $this->data['success']='Bạn vui lòng kiểm tra mail để lấy lại mật khẩu!';   
        }  
        $this->data['title']='Hàng Chính Hiệu - Quên mật khẩu';   
    ...
    function check_mail_forget(){
        ...
        if($this->Mcustomer->customer_detail_email($email))
        {
            ...
            $this->form_validation->set_message(__FUNCTION__, 'Email không tồn tại trên hệ thống cửa hàng');
            return FALSE;
        }
    }
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Hàng Chính Hiệu - Cập nhật mật khẩu mới';
